 ...
 *  - "PSX Indices" (default): the 10 PSX benchmark indices with
 *    constituent drill-down, from /api/market/indices-full +
 *    /api/market/index-constituents. This view's rendering, data
 *    fetching, sort, and drill-down are unchanged from before the tabs
 *    were added — it was extracted into PsxIndicesView verbatim.
 *  - "Global Futures" (new, additive): PMEX index FUTURES that reference
 *    global benchmarks — NOT spot index levels. See GlobalFuturesView.
 */
 ...
 * (eyebrow/heading/tabs) moved up into IndicesPage.
 */
function PsxIndicesView() {
...
/** Columns for the Global Futures table. */
...
/**
 * PMEX index-FUTURES table. These are PMEX-listed futures contracts that
 * reference global benchmarks — NOT spot index levels — so every row is
 * labelled "PMEX futures" and a note below the table states the
 * distinction plainly (same factual-not-overstated discipline as the
 * KMI-30 note / PSX Value omission). Simple table, no drill-down —
 * futures contracts have no constituents.
 */
function GlobalFuturesView() {
  const { data, loading, error } = useGlobalFutures();
  ...
  return (
    <>
      <div className="liquid-glass glass-sheen mt-8 overflow-hidden rounded-3xl">
        {error && !futures ? (
          <div className="px-6 py-16 text-center text-sm text-gray-400">
            Futures data is temporarily unavailable. Please try again shortly.
          </div>
        ) : loading && !futures ? (
          <div className="px-6 py-16 text-center text-sm text-gray-400">
            Loading live futures…
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] border-collapse text-sm">
              ...
              <tbody>
                {(futures ?? []).map((f: GlobalFuturesQuote) => (
                  <tr
                    key={f.contract}
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <span className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-400">PMEX futures</span>
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-right tabular-nums">
                      <ChangeCell value={f.changePoints} suffix="" />
                    </td>
                    <td className="px-5 py-3.5 text-right tabular-nums">
                      <ChangeCell value={f.changePercent} suffix="%" />
                    </td>
                    <td className="px-5 py-3.5 text-right tabular-nums text-gray-300">
                      {fmtVolume(f.volume)}
                    </td>
                    <td className="px-5 py-3.5 text-right tabular-nums text-gray-300">
                      {fmtNum(f.high ?? undefined)}
                    </td>
                    <td className="px-5 py-3.5 text-right tabular-nums text-gray-300">
                      {fmtNum(f.low ?? undefined)}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {futures && (
        <p className="mt-4 max-w-3xl text-xs leading-relaxed text-gray-400/90">
          These are <span className="font-semibold text-gray-300">futures
          contracts</span> listed on the Pakistan Mercantile Exchange
          (PMEX) — one of the two exchanges AZEE is a member of. Each
          references the named global benchmark (e.g. the S&amp;P 500); the
          prices shown are the PMEX futures bid/ask, not the spot index
          level. High and low show &ldquo;—&rdquo; when PMEX has not
          reported them outside trading hours.
        </p>
      )}
    </>
  );
...
const TABS: { id: Tab; label: string }[] = [
  { id: "psx", label: "PSX Indices" },
  { id: "global", label: "Global Futures" },
];

const TAB_EYEBROW: Record<Tab, string> = {
  psx: "Pakistan Stock Exchange",
  global: "Pakistan Mercantile Exchange",
};

const TAB_INTRO: Record<Tab, string> = {
  psx: "Live values for the Pakistan Stock Exchange's benchmark indices — current level, session high and low, change, and traded volume. Select any index to see its constituent companies. Data from the Pakistan Stock Exchange.",
  global:
    "PMEX index futures — contracts traded on the Pakistan Mercantile Exchange that reference major global benchmarks. These are futures prices, not spot index levels.",
};
...
export function IndicesPage() {
  ...
  /*
   * Tab lives in the URL (?tab=global) so a view can be linked/shared and
   * back/forward moves between them. PSX is the default, so the existing
   * Tools-dropdown and Footer links to /indices land where they always
   * have. Changing only the query string leaves pathname/hash untouched,
   * so ScrollToHash doesn't fire.
   */
  const [searchParams, setSearchParams] = useSearchParams();
  const tab: Tab = searchParams.get("tab") === "global" ? "global" : "psx";
    ...
    <main className="min-h-screen text-white">
      ...
      <section className="section-tint-a relative px-4 pb-20 pt-[calc(var(--nav-height)+2.5rem)] sm:px-6 lg:px-12">
        <div className="mx-auto max-w-7xl">
          ...
          {tab === "psx" ? <PsxIndicesView /> : <GlobalFuturesView />}
        </div>
      </section>
