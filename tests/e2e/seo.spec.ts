...
const ROUTE_TITLES: [path: string, title: RegExp][] = [
  ...
  ["/etfs", /^PSX ETFs — Live Exchange Traded Fund Prices \| AZEE Trade$/],
  ["/forex", /^Forex — Live PKR Exchange Rates \| AZEE Trade$/],
  [
    "/announcements",
...
test("sitemap.xml is valid and lists every real route, module slugs included", async ({
  ...
  // 26 URLs total — nine top routes, eight modules, nine legal pages.
  expect(body.match(/<loc>/g)?.length).toBe(26);
});
